...
const GENRES = ['Catálogo Completo', 'Novedades', ...GENRE_LABELS];
let activeGenre = 'Catálogo Completo';
let genreBeforeSearch = 'Catálogo Completo'; // restores on clear
let searchQuery = '';
...
loadCatalog()
    .then(data => {
        CATALOG = data;
        // Total de discos EN STOCK = disponibles ahora (excluye "Pronto en Stock").
        const stockEl = document.getElementById('stockNum');
        if (stockEl) stockEl.textContent = CATALOG.filter(v => v.available).length;
        render();
    })
    .catch(() => {
